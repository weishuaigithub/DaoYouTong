...
/// 地图比例尺级别，在手机上当前可使用的级别为3-21级  比例尺级别 
...
/// MapView的Delegate，mapView通过此类来通知用户对应的事件 用户事件
...
/**
 *当选中一个annotation views时，调用此接口  ！（选中标注）
 *@param mapView 地图View
...
/**
 *当点击annotation view弹出的泡泡时，调用此接口  点击标注上的文字泡泡
 *@param mapView 地图View
...
/**
 *点中覆盖物后会回调此接口，目前只支持点中BMKPolylineView时回调 点中覆盖物
 *@param mapView 地图View
